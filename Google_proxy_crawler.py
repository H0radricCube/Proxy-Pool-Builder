...
class Google_proxy_crawler:
    # you'll need a random free proxy as your search_word e.g. 47.90.63.202
    def __init__(self, search_word = "188.166.204.196"):
        self.Net = net_fn.Net()
        self.search_word = search_word

    def Search_Page(self,keyword,page_num):
        url = "https://www.google.com/search?q={}&start={}".format(keyword,page_num*10)
        rs = self.Net.Get(url)
        end = []
        try:
            ...
            return []
        for searchWrapper in soup.find_all('div', {'class':'BNeawe UPmit AP7Wnd'}): # wtf ? can't get the same html as inspect
    ...
    def build_proxy_pool(self, new_pool = True, max_page_num = 10):
        ...
        proxy_Reader = Proxy_Reader.Proxy_Reader(proxy_source_list, self.search_word)
