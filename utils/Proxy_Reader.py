...
from . import net_fn
import json, urllib3, time, datetime
...
time_stamp = datetime.datetime.utcfromtimestamp(time.time()).strftime("%Y-%m-%d")
...
class Proxy_Reader:
    ...
    def Check_List_Proxy_Stat(self,proxy_source_list):
        ...
        for source_link in proxy_source_list:
            ...
            t = Thread(target=self.Check_Dump_Page,args=[source_link, dumpdir])
    ...
    def Check_Dump_Page(self, url, dumpdir, dump = True):
        ...
        if dump and rs_stat:
            # with open('./math_term_spider/proxy_pool/data/' + self.search_word + '.json', 'a') as pool_data:
            with open(dumpdir +'/' + self.search_word + '_' + time_stamp + '.json', 'a') as pool_data:
                write_data = '\n'.join(proxy_list) + '\n'
